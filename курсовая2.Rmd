---
title: "курсовая_чистовой вариант"
...
data = apply_labels(data,
                      indinc = "Доход домохозяйства/Кол-во людей ",
                      sex = "Пол респондента",
                      sex = c("Мужчины" = 0,
                             "Женщины" = 1),
                      x_age = "Возраст респондента",
                      bmi = "ИМТ",
                      educ = "Высшее образование",
                      educ = c("Нет" = 0,
                             "Да" = 1),
                      mar_status = "Семейное положение",
                      mar_status = c("Без партнера" = 0,
                             "С партнером" = 1)
)
```
...
```{r}
#добавляю пропущенных в доход
data$inc2= ifelse((is.na(data$indinc) & (data$stroke==1 | data$heart_attack==1)), data$xj60, data$indinc )

# Пропуски по доходу 
data=data %>%tidyr::drop_na(inc2)

# Описательная статистика до чистки выбросов 
table1(~ x_age+factor(educ) + factor(mar_status) + factor(labour_status)+ inc2 + factor(region1) + factor(city) + factor(smoker)+ pack_years2  + factor(ph_activity)+ bmi+ factor(weight_status) +factor(alcohol_intake)+ factor(diabetes)+ factor(hypertension) + factor(depression) + factor(stress_event)+ factor(stroke) + factor(heart_attack) |factor(sex) , data=data)

# Оставляем только нужные переменные
w=data%>%select(idind, x_age, educ, mar_status, region1, labour_status, is_working,indinc, xj60,inc2, city, smoker, pack_years2 , ph_activity, bmi, weight_status, alcohol_intake, diabetes, hypertension, depression, stress_event, stroke,  heart_attack, sex)

# Переходим в Стату
write.dta(w, file = "wf.dta")

data%>% 
  ggplot()+
  geom_density (aes(x = log(inc2))) + 
  xlab(" ") + 
  ylab("  ") + 
...
```{r}
# Загружаем данные после очистки зарплаты от выбросов
data_removed_outliers <- read_dta("/Users/adelyashpitser/Desktop/data_removed_outliers.dta")

# Финальная описательная статистика, представленная в тексте КР
table1(~ x_age+factor(educ) + factor(mar_status) + factor(labour_status)+ wage+ log_wage  + factor(region1) + factor(city) + factor(smoker) + factor(ph_activity)+ bmi+ factor(weight_status) +factor(alcohol_intake)+ factor(diabetes)+ factor(hypertension) + factor(depression) + factor(stress_event)+ factor(stroke) + factor(heart_attack) |factor(sex) , data=data_removed_outliers)
...
m1=w%>% 
  ggplot() +
    geom_boxplot(aes(y =log(inc2)), fill="cadetblue3", alpha=0.5)+ 
    xlab("До чистки выбросов") + 
    ylab("Логарифм дохода") + 
    ggtitle("Распределение логарифма дохода") + 
    theme_minimal()

m2=data_removed_outliers%>% 
  ggplot() +
    geom_boxplot(aes(y =log_wage), fill="cadetblue3", alpha=0.5)+ 
    xlab("После") + 
    ylab("") + 
    ggtitle("") + 
    theme_bw()
...
k1=w%>%ggplot(aes(x = log(inc2))) +
              geom_histogram (aes(y = ..density..), fill="cadetblue3", alpha=0.5, binwidth =0.06) + 
              labs(title = " ",
               x = " ", 
               y = "Log of wage after removing outliers",
              fill = " ") + theme(legend.position = "none")+
              geom_density(alpha = .2, fill = "white")+ 
              theme_bw()

k2=data_removed_outliers%>% ggplot(aes(x = log_wage)) +
              geom_histogram (aes(y = ..density..), fill="cadetblue3", alpha=0.5, binwidth =0.06) + 
              labs(title = " ",
               x = " ", 
               y = "Log of wage after removing outliers",
...
data_removed_outliers%>%
  ggplot()+
  geom_density (aes(x = log_wage)) + 
  xlab(" ") + 
  ylab("  ") + 
  ggtitle("Probability density function") + 
  theme_bw()+
  geom_line(data = data_removed_outliers, aes(x=mean(log_wage), y=0), color='black', linetype='dotted')

data_removed_outliers%>%
  ggplot()+
  geom_histogram (aes(x = log_wage), binwidth =0.06) + 
  xlab(" ") + 
  ylab("  ") + 
  ggtitle("Probability density function") + 
  theme_bw()
